...
- **스레딩:** `std::thread` 기반 row-chunk 병렬 처리 (`parallel_for_rows`)
...
`matmul`, `matmul_strassen`, `solve`, `inverse`, `inverse_schur`, `frobenius_norm`, `residual_norm`, `condition_number`, `add`, `subtract`, `hadamard`, `scalar_mul`, `transpose`, `neg`, `hardware_backend`, `trace`, `det`
